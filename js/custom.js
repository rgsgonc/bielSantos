		...
		$('#contactForm').submit(function () {
			...
			$('#contactForm .requiredField').each(function () {
				if (jQuery.trim($(this).val()) === '') {
					...
					$('#contactForm span').html('<strong>*Por favor preencha todos os campos.</strong>');
					hasError = true;
					...
					if (!emailReg.test(jQuery.trim($(this).val()))) {
						var labelText = $(this).prev('label').text();
						$(this).addClass('fielderror');
						$('#contactForm span').html('<strong>O seu endereço de e-mail está incorreto.</strong>');
						hasError = true;
					}
				}
			});
			if (!hasError) {
				$('#contactForm').slideDown('normal', function () {
					...
					$("#contactForm #sendMessage").attr("disabled", "disabled").val('Enviando mensagem. Por favor aguarde...');
				});
				var formInput = $(this).serialize();
				$.post($(this).attr('action'), formInput, function (data) {
					$('#contactForm').slideUp("normal", function () {
						$(this).before('<div class="notification-box notification-box-success"><p><i class="icon-ok"></i>Obrigado!</strong> Seu e-mail foi enviado com sucesso. Assim que verificarmos seu e-mail logo estaremos respondendo.</p></div>');
					});
				});
			}
			return false;
